$(function () {
    ...
    $.fn.InitUploader = function (p) {
        var fun = function (parentObj) {
            ...
            //初始化属性
            var sendurl = p.sendurl;

            /*sendurl += "?action=UpLoadFile&shopFile=1234";
            if (p.water) {
                sendurl += "&IsWater=1";
            }
            if (p.thumbnail) {
            ...
            uploader.on('fileQueued', function (file) {
                //如果是单文件上传，把旧的文件地址传过去
                if (!p.multiple) {
                    uploader.options.formData.DelFilePath = parentObj.siblings(".upload-path").val(file.name);
                }
                //防止重复创建
            ...
            uploader.on('uploadSuccess', function (file, data) {
                ...
                if (data.status == '1') {
                    ...
                    if (!p.multiple) {
                        ...
                        $('.preview .current .img img').attr("src", data.path);
                    } else {
